...
class eighthItem():
    ...
    def __init__(self, continentColumn, continentName, countryColumn, countryName, infectedColumn, dayColumn, predictionYear, data):
        ...
        self.predictionYear = predictionYear
    ...
    def analysis(self):
        ...
        formatedDate = '1-1-' + str(self.predictionYear)
        try:
        ...
        formatedDate = '31-12-' + str(self.predictionYear)
        try:
            formatedDate = datetime.strptime(formatedDate, '%d-%m-%Y')
        except:
            formatedDate = datetime.strptime(formatedDate, '%Y-%m-%d')
        ...
        jsonString = self.generateJSON(labels, setValues, predictedValues, formatedDate, prediction1, prediction2, mse, r2, coef, intercept)
        return jsonString

    def generateJSON(self, labels, setValues, predictedValues, formatedDate, prediction1, prediction2, mse, r2, coef, intercept):
        labelsOutput = '"labels": ['
        contador = 0
        for label in labels:
            if contador == 0:
        ...
        conclutionOutput = self.generateConclution(formatedDate, prediction1, prediction2, mse, r2, coef, intercept)
        output = '{' + labelsOutput + setValuesOutput + predictedValuesOutput + graphName + conclutionOutput + '}'
        return json.loads(output)

    def generateConclution(self, formatedDate, prediction1, prediction2, mse, r2, coef, intercept):
        output = '"conclution": {'
        ...
        leftColumn += '\\nLa prediccion obtenida para la fecha 1-1-' + str(self.predictionYear) + ' es de ' + str(prediction1) + ' infectados. '
        leftColumn += '\\nMientras que la prediccion obtenida para la fecha 31-1-' + str(self.predictionYear) + ' es de ' + str(prediction2) + ' infectados. '
        leftColumn += '\\nPor lo que se espera que en para el ' + str(self.predictionYear) + ' se agreguen ' + str(prediction2 - prediction1) + ' infectados", '
